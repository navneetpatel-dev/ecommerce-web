...
import { getApiErrorMessage, sanitizeUserFacingMessage } from '@/shared/utils/apiErrorMessage'
...
type CropSession = {
  ...
  pending: Array<{ objectUrl: string; filename: string; mimeType: ImageMimeType }>
}
...
export function FileUpload(props: FileUploadProps) {
  ...
  const maxBytes = props.maxBytes ?? maxBytesForUpload(props.entityType, props.purpose)
  const accept = props.accept ?? getAcceptForUpload(props.entityType, props.purpose)
        ...
        result.errors
          .map((e) =>
                formatLabel(LABELS.uploadFileFailedAt, {
                  ...
                  message: sanitizeUserFacingMessage(e.message, LABELS.uploadFailed),
                }),
          )
          .join(' '),
      )
  ...
  const previewEntries: PreviewEntry[] = isMultiple
    ...
    : (props as SingleProps).valueUrl
      ? [
          previews.find((p) => p.storedUrl === (props as SingleProps).valueUrl) ?? {
