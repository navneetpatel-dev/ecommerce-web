...
import { isAdminRole, isCustomerRole, isVendorRole } from '@/shared/utils/roles'
import {
...
export function profileBasePathForRole(role: RoleName | null | undefined): string {
  if (isAdminRole(role)) return PATHS.admin.profile
  if (isVendorRole(role)) return PATHS.vendor.profile
  return PATHS.profile
}

export function profileDefaultTabForRole(role: RoleName | null | undefined): string {
