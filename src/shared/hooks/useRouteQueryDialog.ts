...
import { QUERY_FLAG, QUERY_PARAMS } from '@/shared/constants/queryParams'
import { replaceRouteQuery } from '@/shared/utils/routeQuery'
...
export function useRouteQueryDialog(options: UseRouteQueryDialogOptions = {}) {
  ...
  const mode: RouteQueryDialogMode | null = editId ? 'edit' : isCreate ? 'create' : null
  const open = mode !== null

  const setQuery = useCallback(
    (updates: Record<string, string | null | undefined>) => {
      router.replace(replaceRouteQuery(pathname, searchParams, updates), { scroll: false })
    },
