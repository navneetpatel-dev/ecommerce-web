...
export const bugReportKeys = {
  ...
  mine: (filters: BugListParams = {}) => [...bugReportKeys.all, 'mine', filters] as const,
  admin: (filters: BugListParams = {}) => [...bugReportKeys.all, 'admin', filters] as const,
...
export function useBugCommentsInfinite(id: string, options: { enabled?: boolean } = {}) {
  const authEnabled = useEnabled()
  return useInfiniteQuery({
    queryKey: bugReportKeys.comments(id),
    queryFn: ({ pageParam }) =>
      bugReportsApi.listComments(id, { limit: DEFAULT_PAGE_LIMIT, cursor: pageParam }),
    initialPageParam: null as string | null,
    getNextPageParam: (lastPage) => lastPage.nextCursor,
...
export function useCreateBugReport() {
  ...
  return useMutation({
    mutationFn: (input: { body: CreateBugReportBody; pageUrl?: string | null }) =>
...
export function useUpdateBugStatus(id: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (status: BugReportStatus) => bugReportsApi.updateStatus(id, status),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: bugReportKeys.detail(id) })
      void queryClient.invalidateQueries({ queryKey: bugReportKeys.all })
    },
...
export function useAddBugComment(id: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: bugReportKeys.comments(id) })
      void queryClient.invalidateQueries({ queryKey: bugReportKeys.detail(id) })
    },
  })
}
