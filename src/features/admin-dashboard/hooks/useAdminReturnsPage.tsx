    ...
    if (status === RETURN_STATUS.APPROVED || status === RETURN_STATUS.REFUNDED) {
      buttons.push(
        <AdminConfirmAction
          ...
          description={formatLabel(LABELS.confirmSchedulePickupBody, { name })}
          onConfirm={() =>
            returnsApi.transition(String(row.id), RETURN_STATUS.PICKUP_SCHEDULED).then(reload)
          }
        />,
      )
    }
    ...
    if (status === RETURN_STATUS.PICKUP_SCHEDULED) {
      buttons.push(
        <AdminConfirmAction
          ...
          onConfirm={() =>
            returnsApi.transition(String(row.id), RETURN_STATUS.RECEIVED).then(reload)
          }
        />,
      )
    }

    if (status === RETURN_STATUS.RECEIVED || (status === RETURN_STATUS.REFUNDED && row.receivedAt)) {
      buttons.push(
        <AdminConfirmAction
          key="close"
          ...
          onConfirm={() =>
            returnsApi.transition(String(row.id), RETURN_STATUS.CLOSED).then(reload)
          }
        />,
      )
    }
