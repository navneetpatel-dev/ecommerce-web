...
export function AnalyticsRatingChart({ data }: AnalyticsRatingChartProps) {
  ...
  const chartData = data.map((row) => ({
    ...row,
    label: formatLabel(LABELS.analyticsStarRating, { rating: String(row.rating) }),
  }))
  ...
  return (
    <Card className="h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-[1.0625rem]">{LABELS.analyticsRatingDistribution}</CardTitle>
      </CardHeader>
      <CardContent>
        {!hasData ? (
          ...
          <div className="h-56 w-full sm:h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <CartesianGrid stroke={colors.line} strokeDasharray="3 6" horizontal={false} />
                <XAxis
                  type="number"
                  allowDecimals={false}
