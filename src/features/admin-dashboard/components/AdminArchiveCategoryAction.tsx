...
function ArchiveDescription({ categoryId, name }: { categoryId: string; name: string }) {
  const [count, setCount] = useState<number | null>(null)
  useEffect(() => {
    void categoriesApi.productCount(categoryId).then((result) => setCount(result.productCount))
  }, [categoryId])
  return (
    <>
...
export function AdminArchiveCategoryAction({ category, onDone }: AdminArchiveCategoryActionProps) {
  ...
  return (
    <AdminConfirmAction
      ...
      title={isArchived ? LABELS.confirmReactivateCategoryTitle : LABELS.confirmArchiveCategoryTitle}
      description={
        isArchived ? (
          formatLabel(LABELS.confirmReactivateCategoryBody, { name: category.name })
        ) : (
      ...
      onConfirm={() =>
        categoriesApi
          .update(category.id, {
            status: isArchived ? CATEGORY_STATUS.ACTIVE : CATEGORY_STATUS.ARCHIVED,
          })
          .then(onDone)
      }
    />
