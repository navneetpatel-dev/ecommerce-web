      ...
      <ul className="space-y-1">
        {payload.map((entry) => {
          ...
          return (
            <li
              key={key}
              className="flex items-center justify-between gap-6 text-[0.8125rem] text-ink-muted"
            >
              <span className="flex items-center gap-2">
                <span
                  ...
                  aria-hidden
                />
                {isRevenue ? LABELS.analyticsRevenueSeries : LABELS.analyticsOrdersSeries}
              </span>
...
export function AnalyticsTrendChart({ data }: AnalyticsTrendChartProps) {
  ...
  const chartData = useMemo(() => fillAnalyticsTrendDays(data ?? [], 30), [data])
  const tickLabels = useMemo(() => {
    const indexes = pickTrendTickIndexes(chartData.length, 7)
    return indexes.map((index) => chartData[index]?.label).filter(Boolean) as string[]
  }, [chartData])
  ...
  return (
    <Card className="h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-[1.0625rem]">{LABELS.analyticsOrderTrend}</CardTitle>
        <CardDescription>{LABELS.analyticsOrderTrendHint}</CardDescription>
      </CardHeader>
      <CardContent className="pt-2">
        {chartData.every((point) => point.count === 0 && point.revenue === 0) ? (
          <p className="py-16 text-center text-[0.9375rem] text-ink-muted">
          ...
          <div className="h-72 w-full min-w-0 sm:h-80">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <ComposedChart
                ...
                barCategoryGap="18%"
              >
                <CartesianGrid stroke={colors.line} strokeDasharray="3 6" vertical={false} />
                <XAxis
                  dataKey="label"
                  ticks={tickLabels}
                ...
                <Legend
                  ...
                  align="right"
                  iconType="circle"
                  wrapperStyle={{ paddingBottom: 8, fontSize: 12, color: colors.inkMuted }}
                />
                <Bar
                  yAxisId="orders"
                  dataKey="count"
