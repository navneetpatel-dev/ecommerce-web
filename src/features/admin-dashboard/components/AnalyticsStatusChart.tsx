    ...
    <Card className="h-full">
      ...
      <CardContent>
        {data.length === 0 || total === 0 ? (
          <p className="py-12 text-center text-[0.9375rem] text-ink-muted">
          ...
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-stretch">
            <div className="relative h-44 w-full max-w-[11.5rem] shrink-0 sm:h-48">
                ...
                <PieChart>
                  ...
                  >
                    {data.map((entry, index) => (
                      <Cell key={entry.status} fill={sliceColor(colors, index)} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                <p className="font-mono text-[1.25rem] font-semibold text-ink">{total}</p>
                  ...
                  <li key={row.status} className="flex items-center justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-2">
                      <span
                        ...
                        aria-hidden
                      />
                      <StatusBadge status={row.status} className="max-w-full truncate" />
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="font-mono text-[0.875rem] font-medium text-ink">{row.count}</p>
                      <p className="text-[0.6875rem] text-ink-faint">{share}%</p>
                    </div>
                  </li>
                )
              })}
