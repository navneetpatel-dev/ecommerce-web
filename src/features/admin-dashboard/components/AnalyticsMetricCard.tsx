  ...
  return (
    <Card
      className={cn(
        ...
        tone === 'brand' && 'border-brand/25 bg-gradient-to-br from-brand-subtle/80 to-surface',
      ...
      <CardContent className="flex flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={cn(
              ...
              tone === 'brand' ? 'bg-brand/15 text-brand' : 'bg-paper text-ink-muted',
            )}
          >
        ...
        {(hint || trend != null) && (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[0.75rem]">
            ...
            {trend != null && !hint && (
              <span className="text-ink-faint">{LABELS.analyticsVsPriorPeriod}</span>
            )}
          </div>
        )}
      </CardContent>
