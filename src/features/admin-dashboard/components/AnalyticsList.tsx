...
export function AnalyticsList({ items, labelKey }: AnalyticsListProps) {
  if (!items?.length) return <p className="text-ink-muted text-[0.9375rem] py-2">No data</p>

  return (
    <div className="space-y-2">
      {items.map((item) => (
        <div key={item.id} className="flex justify-between text-[0.9375rem] py-2 border-b border-line last:border-0">
