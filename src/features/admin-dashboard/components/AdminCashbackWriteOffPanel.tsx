        ...
        <FormFieldFrame label={LABELS.reportBornBy} htmlFor="writeoff-born-by">
          <Select value={bornBy} onValueChange={(v) => setBornBy(v as typeof bornBy)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="PLATFORM">{LABELS.reportBornByPlatform}</SelectItem>
        ...
        <>
          <div className="grid gap-4 rounded-md border border-line bg-surface p-4 sm:grid-cols-2">
            <div className="space-y-1">
              <p className="text-[0.8125rem] text-ink-muted">{LABELS.reportRecoveredTotal}</p>
              <p className="text-[1.125rem] font-semibold tabular-nums text-success">
                {formatInr(report.recoveredTotal)}
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-[0.8125rem] text-ink-muted">{LABELS.reportWrittenOffTotal}</p>
