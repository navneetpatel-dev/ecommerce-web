...
export function VendorsIndexView({ vendors, isLoading, isEmpty }: VendorsIndexViewProps) {
  return (
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="rounded-lg border border-line bg-paper/40 p-4">
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[0.9375rem] font-semibold text-ink transition-colors group-hover/tile:text-brand">
                      {vendor.businessName}
                    </span>
                    {vendor.description ? (
                      <span className="mt-1 line-clamp-2 text-[0.8125rem] text-ink-muted">
                        {vendor.description}
                      </span>
                    ) : null}
                  </span>
                </div>
                <span className="mt-4 inline-flex items-center gap-1 text-[0.8125rem] font-medium text-brand">
                  {LABELS.visitShop}
