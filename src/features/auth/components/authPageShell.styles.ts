export const authPageShellStyles = {
  root: "relative isolate min-h-[100dvh] overflow-x-clip bg-paper",
  bgGradient1:
    "pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_90%_70%_at_50%_0%,color-mix(in_srgb,var(--brand-subtle)_55%,transparent),transparent_62%),radial-gradient(ellipse_80%_60%_at_100%_100%,color-mix(in_srgb,var(--accent-subtle)_40%,transparent),transparent_58%)]",
  bgGradient2:
  ...
  layoutGrid:
    "relative grid min-h-[100dvh] w-full lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.95fr)] lg:gap-8 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.95fr)] xl:gap-10 2xl:gap-14",
  brandAside:
  ...
  brandEyebrow:
    "text-[0.6875rem] font-semibold uppercase tracking-[0.22em] text-brand",
  brandHeadline:
    "font-display text-[clamp(1.75rem,3.2vw,3.5rem)] leading-[1.08] tracking-tight text-ink",
  brandBody:
    "max-w-lg text-body-lg leading-relaxed text-ink-muted 2xl:text-[1.125rem]",
  ...
  formWrapper:
    "relative w-full max-w-[min(100%,28rem)] xl:max-w-[min(100%,26rem)] 2xl:max-w-[min(100%,28rem)]",
  formAnimation: "relative motion-safe:animate-[auth-rise_420ms_ease-out_both]",
  formGlow:
    "pointer-events-none absolute -inset-3 hidden rounded-[1.25rem] bg-[radial-gradient(ellipse_at_center,color-mix(in_srgb,var(--brand-subtle)_35%,transparent),transparent_72%)] opacity-80 sm:block sm:-inset-4",
} as const;
