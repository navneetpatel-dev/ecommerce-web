    ...
    return (
      ...
      >
        {FEATURES.map(({ icon: Icon, title }) => (
          <li
            key={title}
            className="inline-flex items-center gap-1.5 rounded-full border border-line/80 bg-surface/55 px-3 py-1.5 text-[0.75rem] font-medium text-ink-muted shadow-elevation-1 backdrop-blur-sm sm:px-3.5 sm:text-body-sm"
          >
            <Icon
              aria-hidden
  ...
  return (
    ...
    >
      {FEATURES.map(({ icon: Icon, title, hint }) => (
        <li
          key={title}
          className="flex items-start gap-3 px-3.5 py-3 md:px-4 md:py-3.5 lg:py-4"
        >
