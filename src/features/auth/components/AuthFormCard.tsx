...
export function AuthFormCard({ title, description, children, footer, className }: AuthFormCardProps) {
  return (
    ...
    >
      ...
      <div className="space-y-5 px-5 py-5 sm:space-y-6 sm:px-8 sm:py-8">{children}</div>
      {footer ? (
        <footer className="border-t border-line/80 bg-paper/30 px-5 py-4 sm:px-8 sm:py-5">{footer}</footer>
      ) : null}
    </div>
  )
}
