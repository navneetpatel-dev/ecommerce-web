  ...
  return (
    <div className="storefront-container py-8 md:py-10">
      <header className="mb-8 max-w-2xl">
        <h1 className="font-display text-[1.75rem] text-ink md:text-[2rem]">Your reviews</h1>
        ...
        <ul className="divide-y divide-line border border-line bg-surface-raised">
          {reviews.map((review) => (
            <li key={review.id} className="px-5 py-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 flex items-center gap-1 text-[0.8125rem] text-ink-muted">
                    ...
                    {review.rating}/5
                    <span className="text-ink-faint">· {formatOrderDate(review.createdAt)}</span>
                  </p>
                </div>
              ...
              <p className="mt-1 text-[0.9375rem] text-ink-muted whitespace-pre-wrap">{review.body}</p>
            </li>
          ))}
        </ul>
      )}
