...
const WORKSPACE_ACCOUNT_SECTION_IDS: AccountSectionId[] = ['personal', 'security', 'privacy']

export function accountSectionsForRole(role: RoleName | null | undefined): AccountNavItem[] {
...
export function workspaceAccountSections(): AccountNavItem[] {
  return ACCOUNT_SECTIONS.filter((section) => WORKSPACE_ACCOUNT_SECTION_IDS.includes(section.id)).map(
    (section) =>
      section.id === 'privacy'
...
export const DEFAULT_WORKSPACE_ACCOUNT_SECTION: AccountSectionId = 'personal'
