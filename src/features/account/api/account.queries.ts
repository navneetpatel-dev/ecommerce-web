...
export function useUpdateAccountAddress() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ addressId, body }: { addressId: string; body: Partial<AddressInput> }) =>
      accountApi.updateAddress(addressId, body),
    onSuccess: (updated) => {
      patchAddressCache(queryClient, (list) => {
        const next = list.map((a) => (a.id === updated.id ? updated : a))
        if (updated.isDefault) {
          return next.map((a) => (a.id === updated.id ? a : { ...a, isDefault: false }))
        }
        return next
      })
    },
...
export function useDeleteAccountAddress() {
  ...
  return useMutation({
    ...
    onSuccess: (_void, addressId) => {
      patchAddressCache(queryClient, (list) => list.filter((a) => a.id !== addressId))
    },
  })
}
