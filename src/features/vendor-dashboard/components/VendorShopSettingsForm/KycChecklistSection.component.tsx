...
export function KycChecklistSection({
  ...
  return (
    <FormSection title={LABELS.kycChecklist} hint={LABELS.kycChecklistHint}>
      ...
      {checklistError ? (
        <p className="sm:col-span-2 text-body-sm text-danger">
          {checklistError}
        </p>
      ) : null}
        ...
        {items.map((item) => (
          <li key={item.documentType}>
            <Button
              ...
              className={`h-auto min-h-0 max-h-none w-full justify-between gap-3 px-3 py-2 text-left text-body-sm font-normal ${
