...
import { VendorReviewRespondDialog } from './VendorReviewRespondDialog'
  ...
  return (
    <section className="w-full min-w-0 space-y-6">
      ...
      {isLoading ? (
        <div className="space-y-6" aria-busy="true" aria-live="polite">
          {Array.from({ length: 3 }).map((_, index) => (
            <Skeleton key={index} className="h-[8.5rem] w-full rounded-md sm:h-[9.5rem]" />
          ))}
        </div>
      ) : loadError ? (
      ...
      {!isLoading && !loadError
        ...
        <article
          key={review.id}
          className="w-full overflow-hidden rounded-md border border-line bg-surface p-4 shadow-[0_1px_0_rgba(15,23,42,0.03)] sm:p-6"
        >
          <p className="font-medium text-ink">
            {review.title?.trim() || LABELS.reviewUntitled} ·{' '}
            {formatLabel(LABELS.reviewRatingOutOf, { rating: String(review.rating) })}
          </p>
          {review.product?.name ? (
            <p className="mt-1 text-[0.8125rem] text-ink-muted">{review.product.name}</p>
          ) : null}
          <p className="mt-2 text-[0.9375rem] leading-relaxed text-ink-muted">{review.body}</p>
          <Button
            className="mt-4"
            size="sm"
            variant="secondary"
      ...
      <VendorReviewRespondDialog
        ...
        onSubmit={(response) => {
          if (!targetId) return
          void Promise.resolve(onRespond(targetId, response)).then(() => setTargetId(null))
        }}
      />
    </section>
  )
