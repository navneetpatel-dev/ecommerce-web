...
export function VendorProductCreateForm({
  ...
  const requiredChecks = [
    ...
    { ok: Boolean(values.description.trim()), message: LABELS.enterProductDescription },
  ]
  const canSubmit = allRequiredFieldsMet(requiredChecks) && !loading
  const disableHint = loading ? LABELS.loading : (firstMissingRequiredHint(requiredChecks) ?? '')
  ...
  const highlightsAtMax = values.highlights.length >= PRODUCT_FIELD_LIMITS.HIGHLIGHTS_MAX
  ...
  return (
    <form className="mb-5" onSubmit={handleSubmit}>
      <FormStack>
        ...
        >
          <FormFieldFrame label={LABELS.productName} required error={getError('name')}>
            <Input
          ...
          <FormFieldFrame label={LABELS.pricePlaceholder} required error={getError('price')}>
            <NumberInput
              prefix="₹"
              min={1}
              step={1}
              ...
              disabled={disabled}
              onChange={(value) => patchValues({ price: value == null ? '' : String(value) })}
            />
          </FormFieldFrame>
          <FormFieldFrame
          ...
          >
            <NumberInput
              ...
              value={values.compareAtPrice === '' ? undefined : Number(values.compareAtPrice)}
          ...
          >
            <Select
              ...
              disabled={disabled}
            >
              <SelectTrigger className={cn(getError('categoryId') && 'border-danger')}>
          ...
          >
            <div className="space-y-2">
              {values.highlights.map((item, index) => (
                <div key={`highlight-${index}`} className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      patchValues({
                        highlights: values.highlights.filter((_, rowIndex) => rowIndex !== index),
                      })
                    }
                    aria-label={formatLabel(LABELS.removeNamed, { name: LABELS.productHighlights })}
                  >
                    <Trash2 />
                  </Button>
                </div>
              ...
              <Button
                ...
                onClick={() => patchValues({ highlights: [...values.highlights, ''] })}
              >
                <Plus aria-hidden />
          ...
          <FormFieldFrame label={LABELS.specifications} error={getError('specs')}>
            <div className="space-y-2">
              {values.specs.map((row, index) => (
                <div key={`spec-${index}`} className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_auto]">
                  ...
                  <Input
                    ...
                    onChange={(event) => {
                      const specs = values.specs.map((current, rowIndex) =>
                        rowIndex === index ? { ...current, value: event.target.value } : current,
                      )
                      patchValues({ specs })
                    }}
                  />
                  <Button
                    ...
                    onClick={() =>
                      patchValues({
                        specs: values.specs.filter((_, rowIndex) => rowIndex !== index),
                      })
                    }
                    aria-label={formatLabel(LABELS.removeNamed, { name: LABELS.specifications })}
                  >
                    <Trash2 />
                  </Button>
                </div>
              ...
              <Button
                ...
                onClick={() => patchValues({ specs: [...values.specs, emptySpecRow()] })}
              >
                <Plus aria-hidden />
        ...
        >
          ...
          >
            <Textarea
              ...
              disabled={disabled}
              onChange={(event) => patchValues({ deliveryNote: event.target.value })}
