...
export function CommissionLedgerTable({ commissions }: CommissionLedgerTableProps) {
  return (
    <div>
      <h2 className="mb-4 text-[1.375rem] font-semibold text-ink">Commission Ledger</h2>
        ...
        <Table scrollContainer={false}>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className={TABLE_DATA_CELL_CLASS}>Sale Amount</TableHead>
              <TableHead className={TABLE_DATA_CELL_CLASS}>Rate</TableHead>
              <TableHead className={TABLE_DATA_CELL_CLASS}>Commission</TableHead>
          ...
          <TableBody>
            {commissions?.items?.length === 0 ? (
              ...
              commissions?.items?.map((c) => (
                <TableRow key={c.id}>
                  <TableCell className={cn(TABLE_DATA_CELL_CLASS, 'text-[0.9375rem]')}>
                    {new Date(c.createdAt).toLocaleDateString()}
                  </TableCell>
                  <TableCell className={cn(TABLE_DATA_CELL_CLASS, 'font-mono')}>
                    ₹{c.saleAmount}
                  </TableCell>
                  <TableCell className={TABLE_DATA_CELL_CLASS}>{c.commissionRate}%</TableCell>
                  <TableCell className={cn(TABLE_DATA_CELL_CLASS, 'font-mono')}>
                    ₹{c.commissionAmount}
                  </TableCell>
