...
type VendorProductCreateFormProps = ComponentProps<typeof VendorProductCreateForm>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[min(92vh,48rem)] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{mode === 'edit' ? LABELS.editProductTitle : LABELS.createProduct}</DialogTitle>
        </DialogHeader>
        <p className="text-[0.875rem] text-ink-muted">
          {mode === 'edit' ? LABELS.editProductBody : LABELS.createProductBody}
        </p>
        <VendorProductCreateForm mode={mode} {...formProps} onCancel={onCancel} />
      </DialogContent>
    </Dialog>
  )
}
