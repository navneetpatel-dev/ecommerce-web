...
import { toCouponCreateBody } from '@/features/admin-dashboard/schemas/coupons.schema'

export const vendorCouponKeys = {
  all: ['vendor', 'coupons'] as const,
  list: (page: number) => [...vendorCouponKeys.all, 'list', page] as const,
  analytics: (id: string) => [...vendorCouponKeys.all, 'analytics', id] as const,
...
export function useCreateVendorCoupon(vendorId?: string | null) {
  ...
  return useMutation({
    mutationFn: (body: CouponFormInput) =>
      couponsApi.vendorCreate(toCouponCreateBody(body, { forceVendorId: vendorId ?? undefined })),
    onSuccess: () => {
