...
export function getMatrixVariants(variants: ProductVariant[], matrixKeys: string[]): ProductVariant[] {
  if (!matrixKeys.length) return variants
  return variants.filter((variant) => variantHasMatrixAttributes(variant, matrixKeys))
}
...
export function resolveDefaultVariantSelection(
  ...
  const inStockMatch = matrixVariants.find((variant) => variantIsInStock(variant))
